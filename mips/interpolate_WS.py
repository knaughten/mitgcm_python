...
reg='Weddell' # 'Amundsen' or 'Weddell'
...
abc = 'a'
exp = 'OceanW-hind'
original_sim_name = 'Weddell Sea 1/4th degree'
files = [data_dir+'output_00'+str(n+1)+'.nc' for n in range(6)]

# Loop over the files and create one output file per year - otherwise OOM abounds
for fname in files:
   print('Processing '+fname)
   oce = mp.load_oce_mod_mitgcm(files_T=fname, rho0=1035, teos10=False, region=reg, parallel=True, projection='lat_lon')
   ...
   def put_global_attrs(ds,experiment='TBD', avg_hor_res_73S=0.0, original_sim_name='TBD', ocean_model='TBD', institute='TBD', \
      ...
      ds.attrs['reference'] = 'Naughten et al. 2019, JCli, doi:10.1175/JCLI-D-19-0203.1'                      # Main publication or website describing the simulation or a similar one
      ...
      ds.attrs['bathymetry'] = 'BedMAP2 + Rosier et al. 2018 doi:10.1029/2018JC013982'                     # Bathymetry dataset (specify exact version), e.g. `BedMachine-1.33', `Bedmap2', 
                                                         #    `RTopo-2.0.4', `Merge of Millan et al. (2017) and Bedmap2'
      ds.attrs['ice_shelf_draft'] = 'BedMAP2'                # Dataset for the depth of ice-shelf base (similar to previous)
      ds.attrs['atmosphere'] = 'ERA-Interim'                     # Atmospheric forcing, with a reference, e.g., `ERA5 (Hersbach et al. 2020)',
                                                         #    `ERAint (Dee et al. 2011)', `JRA55do (Tsujino et al. 2018)',
                                                         #    `MARv3.9.3 (Donat-Magnin et al. 2020)'
      ds.attrs['iceberg'] = 'Prescribed freshwater (NEMO G07, Storkey et al., 2018) spread over surface'                        # Method used to account for melting icebergs, with a reference,
      ...
      ds.attrs['sea_ice'] = 'MITgcm sea ice model with modified EVP (Lemieux 2012)'                        # Method used to simulate or prescribe the ocean--sea-ice interaction,
      ...
      ds.attrs['ocean_lateral_bdy'] = 'Southern Ocean State Estimate (Mazloff et al. 2010)'              # Type of lateral boundary conditions, e.g. 'Simulation (Merino et al. 2018)',
      ...
      ds.attrs['vertical_coordinate'] = 'Stretched geopotential (Zstar)'            # e.g. `Geopotential (Z)', `Stretched Geopotential (Zstar)', `Pressure (P)',
      ...
      ds.attrs['advection'] = '3nd order DST flux limiter'                      # Brief description of the momentum and tracer advection schemes 
                                                         #      (centered, third-order with limiter, etc)
      ds.attrs['horizontal_mixing'] = 'GM-Redi with modified coefficients (Visbeck et al. 1997); Leith harmonic viscosity factor 2.0'              # Brief description of how ``horizontal'' mixing was performed 
