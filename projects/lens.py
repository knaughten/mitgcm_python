...
def check_lens_timeseries (num_ens=5, base_dir='./'):
    ...
    file_paths = [pace_file] + ['PAS_LENS'+str(n+1).zfill(3)+'/output/timeseries.nc' for n in range(num_ens)]
    sim_names = ['PACE mean'] + ['LENS ensemble'] + [None for n in range(num_ens-1)]
    colours = ['blue'] + ['grey' for n in range(num_ens)]
    smooth = 24
    start_year = 1920

    for var in var_names:
        read_plot_timeseries_ensemble(var, file_paths, sim_names=sim_names, precomputed=True, plot_mean=True, first_in_mean=False, colours=colours, smooth=smooth, vline=start_year)
