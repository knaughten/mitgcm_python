from mitgcm_python.file_io import netcdf_time
...
def add_attributes (fname, sim, lon_min, lon_max, lat_min, lat_max, zmin, zmax):
    if sim == 'pace':
        ...
        f.write('"Amundsen Sea MITgcm model output forced with ERA5 reanalysis, 1979-2019" '+fname+'\n')
...
def process_timeseries_var (var_name, sim='pace', ts_file='timeseries_final.nc'):
    ...
    if sim == 'pace':
        for n in range(num_ens):
            ...
            fname_out = dir_tmp_pace+'MITgcm_'+region+'_'+depth+'_1920-2013_'+new_var+'PACE'+str(n+1).zfill(2)+'.nc'
            ...
            if first_year == pace_start_year:
                ...
                t_end = (pace_end_year-pace_spinup_year0+1)*months_per_year
    ...
    if sim == 'era5':
        ...
        fname_out = dir_tmp_era5+'MITgcm_'+region+'_'+depth+'_1920-2013_'+new_var+'ERA5.nc'
        ...
        if first_year == era5_start_year:
            ...
            t_end = (era5_end_year-era5_spinup_year0+1)*months_per_year
            f.write('ncks -d time,'+str(t_start)+','+str(t_end)+' -v '+var_name+' '+fname_in+' '+fname_out+'\n')
        # Rename the variable
        f.write('ncrename -v '+var_name+','+new_var+' '+fname_out+'\n')
        # Add some attributes
...
# Hovmollers for each ensemble member (1920-2013) and ERA5 (1979-2019)

# Lat-lon fields for ERA5 (1979-2019)
