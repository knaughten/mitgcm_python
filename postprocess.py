...
from .utils import real_dir, days_per_month, str_is_int, mask_3d, mask_except_ice, mask_land, mask_land_ice, select_top, select_bottom, mask_outside_box, var_min_max, add_time_dim, apply_mask, convert_ismr
...
def make_trend_file (var_name, region, sim_dir, grid_dir, out_file, dim=3, gtype='t', start_year=1920, end_year=None, end_base_year=1949, time_integral_anomaly=False):
    ...
    for m in range(num_ens):
        ...
        if end_year is not None:
            t_end = file_paths.index(sim_dir[m]+'/output/'+str(end_year)+'01/MITgcm/output.nc')
        else:
            t_end = len(file_paths)-1
        file_paths = file_paths[t_start:t_end+1]            
