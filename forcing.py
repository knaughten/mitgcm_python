...
def ukesm_tas_timeseries (out_dir='./'):
    ...
    expt = ['ssp434'] #['piControl', '1pctCO2', 'abrupt-4xCO2', 'historical', 'ssp126', 'ssp245', 'ssp370', 'ssp434', 'ssp585']
    num_expt = len(expt)
    mip = ['ScenarioMIP'] #['CMIP' for  n in range(4)] + ['ScenarioMIP' for n in range(5)]
    start_year = [2015] #[2910] + [1850 for n in range(3)] + [2015 for n in range(5)]
    end_year = [2100] #[3059] + [1999 for n in range(2)] + [2014] + [2100 for n in range(5)]
    ensemble_member = 'r1i1p1f2'
    var = 'tas'
    time_code = 'day'
    out_dir = real_dir(out_dir)
